...
def add_transaction():
    ...
    signature = wallet.sing_transaction(wallet.public_key, recipient, amount)
...
def broadcast_block():
    ...
    if block['index'] == blockchain.chain[-1].index + 1:
        if blockchain.add_block(block):
            ...
            return jsonify({'message': 'Block is invalid'}), 500
...
def resolve_conflicts():
    ...
    return jsonify({
        'message': 'Chain was replaced' if replaced else 'Local chain kept'
    }), 200
...
def get_nodes():
    response = {
        'all_nodes': blockchain.get_pear_nodes()
    }
    return jsonify(response), 200
